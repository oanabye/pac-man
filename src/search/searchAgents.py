...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # lista colțurilor nevizitate
    unvisited = [c for c, v in zip(corners, visited) if not v]

    if not unvisited:
        return 0

    # heuristică: distanța minimă până la un colț + distanțele între colțuri
    # (un heuristic simplu, dar valid și consistent)
    h = 0
    pos = currentPos
    remaining = unvisited.copy()

    # greedy: ia cel mai apropiat colț întotdeauna
    while remaining:
        distances = [(util.manhattanDistance(pos, c), c) for c in remaining]
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    # Use stored distances if available to save computation
    ...
    for food in foodList:
        ...
        if key not in problem.heuristicInfo['distances']:
            # Compute exact maze distance using BFS
